# IndexLoom Graph bounty demo

Target length: 2 minutes 50 seconds. Keep the browser, API logs, and PostgreSQL query tool visible in separate windows before recording.

## 0:00–0:20 — The problem and boundary

Open the empty Create Pipeline page.

> Blockchain data pipelines normally require event decoding, indexing code, infrastructure, and a custom API. IndexLoom turns a focused data request into a live reusable API. In Phase 1 the AI is deliberately constrained: it can configure a reviewed ERC-4626 template, but it cannot write Rust, SQL, dependencies, paths, or shell commands.
...
As the page polls, show the build, package inspection, graph inspection, and live validation stages.
...
Point to the Phase 2 card without demonstrating payments.

> The Graph is load-bearing from historical validation through continuous synchronization. Next, x402 on Hedera can meter these same data routes without changing the underlying pipeline.
